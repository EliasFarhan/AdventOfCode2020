...
int main(int arg, char** argv)
{
    ...
    const auto maxRaycastSize = std::max(grid.size(), grid[0].size());
    do
    {
        change_count = 0;
            ...
            for (int column = 0; column < gridRow.size(); column++)
            {
                ...
                for (int dx = -1; dx <= 1; dx++)
                {
                    for (int dy = -1; dy <= 1; dy++)
                    {
                        if (dx == 0 && dy == 0)
                            continue;
                        for (int factor = 1; factor < maxRaycastSize; factor++)
                        {
                            const int x = row + dx * factor;
                            if (x < 0 || x >= grid.size())
                                break;
                            const int y = column + dy * factor;
                ...
                else if (currentElement == OCCUPIED_SEAT && occupied >= 5)
                {
